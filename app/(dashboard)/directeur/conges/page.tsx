...
import SuperValidationModal from '@/components/modals/conges/SuperValidationModal';
import ViewCongeModal from '@/components/modals/conges/ViewCongeModal';
import { Calendar, Eye, CheckCircle, ShieldAlert, Trash2, Search, Filter, RotateCcw } from 'lucide-react';
...
export default function DirecteurCongesPage() {
  ...
  const [isSuperValidationModalOpen, setIsSuperValidationModalOpen] = useState(false);
  ...
  const { data: conges, isLoading } = useQuery({
    queryKey: ['conges', activeTab, { type: filterType, date_debut: filterDateDebut, date_fin: filterDateFin, departement_id: filterDepartement }],
    queryFn: () => fetchConges({
  ...
  const handleSuperValidation = (conge: Conge) => {
    setSelectedConge(conge);
    setIsSuperValidationModalOpen(true);
  };
        ...
        <div className="flex items-center gap-2">
          ...
          {(conge.etat === 'en_attente' || conge.etat === 'partiellement_valide') && (
            <>
              ...
              <button
                onClick={() => handleSuperValidation(conge)}
                className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                title="Super Validation Admin"
              >
                <ShieldAlert size={18} />
              </button>
            </>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Administration des demandes de congés"
        icon={<Calendar size={28} className="text-orange-600" />}
        actions={
          <Link
            href="/directeur/conges/corbeille"
      ...
      {selectedConge && (
        <>
          ...
          <SuperValidationModal
            isOpen={isSuperValidationModalOpen}
            onClose={() => setIsSuperValidationModalOpen(false)}
            conge={selectedConge}
          />
          <ViewCongeModal
