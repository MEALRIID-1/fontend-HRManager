...
const NOTIFICATION_CONFIG: Record<TypeNotification, { icon: React.ReactNode; color: string; bg: string }> = {
  ...
  ANNIVERSAIRE_EMBAUCHE: { icon: <Clock size={20} />, color: "text-purple-600", bg: "bg-purple-100" },
  DOCUMENT_REQUIS: { icon: <AlertCircle size={20} />, color: "text-indigo-600", bg: "bg-indigo-100" },
...
export default function ManagerNotificationsPage() {
  ...
  const displayedNotifications = activeTab === "non_lues" ? notifications.filter(n => !n.lue) : notifications;
  ...
  return (
    <DashboardLayout title="Notifications" subtitle={`${nbNonLues} non lue${nbNonLues > 1 ? "s" : ""}`}>
      <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={cn(
                ...
                activeTab === "toutes" ? "bg-primary-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              )}
            >
              Toutes
            </button>
            <button
              ...
              className={cn(
                ...
                activeTab === "non_lues" ? "bg-primary-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              )}
            >
              Non lues {nbNonLues > 0 && <span className="bg-danger text-white text-xs font-bold px-2 py-0.5 rounded-full">{nbNonLues}</span>}
            </button>
          </div>
          {nbNonLues > 0 && (
            <Button variant="ghost" size="sm" icon={<Check size={16} />} onClick={markAllAsRead}>
              Tout marquer lu
            </Button>
          )}
        </div>
            ...
            {displayedNotifications.length > 0 ? (
              displayedNotifications.map((notif) => {
                ...
                return (
                  ...
                  >
                    <div className={cn("flex h-10 w-10 items-center justify-center rounded-xl flex-shrink-0", config.bg, config.color)}>
                      {config.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className={cn("font-medium", notif.lue ? "text-slate-700" : "text-slate-900")}>{notif.titre}</p>
                          <p className="text-sm text-slate-500 mt-1">{notif.message}</p>
                        </div>
                        {!notif.lue && <span className="w-2.5 h-2.5 rounded-full bg-primary-500 flex-shrink-0 mt-1.5" />}
                      </div>
                      <div className="flex items-center gap-3 mt-2">
                        <Badge variant="gray" size="sm">{TYPE_LABELS[notif.type]}</Badge>
                        <span className="text-xs text-slate-400">{fromNow(notif.createdAt)}</span>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="text-center py-16">
                ...
                <p className="text-slate-500">{activeTab === "non_lues" ? "Aucune notification non lue" : "Aucune notification"}</p>
              </div>
            )}
          </div>
        </Card>
